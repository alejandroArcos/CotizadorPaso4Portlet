...
import com.tokio.cotizador.paso4.bean.ModoAuxiliar;
...
public class CotizadorPaso4Portlet extends MVCPortlet {
	
	private static final Log _log = LogFactoryUtil.getLog(CotizadorPaso4Portlet.class);
		...
		try {
			...
			switch (infCotizacion.getTipoCotizacion()) {
				case FAMILIAR:
					...
					nombreCotizador = CotizadorPaso4PortletKeys.TITULO_FAMILIAR;
					break;
				case EMPRESARIAL:
					infCotizacion.setPantalla("moduloEmpresarial");
					nombreCotizador = CotizadorPaso4PortletKeys.TITULO_EMPRESARIAL;
					break;
				default:
	...
	private void fQuienEsQuien(RenderRequest renderRequest){
		...
		try {
			...
			WebResource webResource = client.resource("https://qeq.com.mx/datos/qws/access")
					.queryParam("var1", CotizadorPaso4PortletKeys.QQ_USUARIO)
					.queryParam("var2", CotizadorPaso4PortletKeys.QQ_CONTRASEÑA);
			...
			if (Validator.isNull(cookieId)) {
				...
				if (isMoral){
					...
					WebResource	razMor = client.resource(CotizadorPaso4PortletKeys.QQ_URL_MORAL)
							.queryParam("razonsoc",
									HtmlUtil.escape( responseEmision.getDatosCliente().getNombre().toLowerCase()));					
					
					WebResource	rfcMor = client.resource(CotizadorPaso4PortletKeys.QQ_URL_MORAL)
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());

					WebResource	nom_den_rfc = client.resource(CotizadorPaso4PortletKeys.QQ_URL_MORAL)
							.queryParam("razonsoc",
									HtmlUtil.escape( 
					...
					WebResource nomCom = client.resource(CotizadorPaso4PortletKeys.QQ_URL_FISICA)
					...
					WebResource nomRFC = client.resource(CotizadorPaso4PortletKeys.QQ_URL_FISICA)
							.queryParam("nombre", HtmlUtil.escape(responseEmision.getDatosCliente().getNombre().toLowerCase()))
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());					

					WebResource apPRFC = client.resource(CotizadorPaso4PortletKeys.QQ_URL_FISICA)
							.queryParam("paterno", HtmlUtil.escape(responseEmision.getDatosCliente().getAppPaterno().toLowerCase()))
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());					

					WebResource apMRFC = client.resource(CotizadorPaso4PortletKeys.QQ_URL_FISICA)
							.queryParam("materno", HtmlUtil.escape(responseEmision.getDatosCliente().getAppMaterno().toLowerCase()))
							.queryParam("rfc", responseEmision.getDatosCliente().getRfc());
